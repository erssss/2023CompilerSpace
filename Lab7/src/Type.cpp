...
bool FunctionType::checkParam(std::vector<SymbolEntry*> rParamsSE){
    ...
    for (size_t i = 0; i < rParamsSE.size(); i++){
        rparams.emplace_back(rParamsSE[i]->getType());
    }
    ...
    for (size_t i = 0; i < paramsType.size(); i++)
    {
        ...
        typeVec.emplace_back(paramsType[i]);
        typeVec.emplace_back(rparams[i]);
        transItem * item;
        retCode = pairTypeCheck(typeVec);
        switch(retCode){
            case 0:
                item=new transItem();
                rParamsSE[i]->appendTransItem(this->func_name,item);
                break;
            case 1:
                return 0;
                break;
...
Type* ArrayType::getStripEleType(){
    ...
    if(!this->len.empty()){
        ...
        if(new_dimen!=-1){
            ...
            for(size_t i=1;i<len.size();i++){
                (arr->getLenVec()).emplace_back(len[i]);
            }

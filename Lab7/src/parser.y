    ...
    InitStmt SEMICOLON{
        ...
        while(initexp!=nullptr){
            ...
            if(initexp->getId()->getIsArray()){
                ...
                while(index!=nullptr){
                    if(index->getNull()){
                        arrType->getLenVec().emplace_back(0);
                    ...
                    /*z1107 补完常量维度*/
                    /*todo 需要计算最高维度是什么！！*/
                    arrType->getLenVec().emplace_back(constEntry->getIntValue());
                    }
                    dimen++;
                    index=(ExprNode*)index->getNext();
                }
...
WhileStmt
    : WHILE LPAREN Cond RPAREN{
        ...
        whilestmts.emplace_back(now_whilestmt);
...
FuncDecl
    ...
    FuncHead LPAREN FuncFParamList RPAREN SEMICOLON{
        ...
        while(fparam!=nullptr){
            ...
            vec.emplace_back(idse->getType());
            idse->setAddr(fparam->getId()->getOperand());
            vecSe.emplace_back(fparam->getId()->getOperand());
...
FuncDef
    ...
    FuncHead LPAREN FuncFParamList RPAREN{
        ...
        if(se==nullptr){
            ...
            while(exp!=nullptr){
                rparams.emplace_back(exp->getSymbolEntry());
                exp=(ExprNode*)exp->getNext();
            }
            bool check_res=true;
            if(check_on)
...
FuncCall
    ...
    | GETARRAY LPAREN FuncRParam RPAREN{
        ...
        if(se==nullptr){
            ...
            //fix: vec.emplace_back()这里需要push进去int数组的形参
            //z1209 完善指针
            vec.emplace_back(arrType);
            funcType = new FunctionType($1,funchead->getType(), vec);
            se = new IdentifierSymbolEntry(funcType, funchead->getName(), 0);
            globals->install(funchead->getName(), se);
            unit.insertDeclare(se);
    ...
    | GETFARRAY LPAREN FuncRParam RPAREN{
        ...
        if(se==nullptr){
            ...
            Type* funcType;
            std::vector<Type*> vec;
             //vec.emplace_back()这里需要push进去float数组的形参
    ...
    | PUTFLOAT LPAREN FuncRParam RPAREN{
        *fcheck=1;
        ...
        if(se==nullptr){
            ...
            Type* funcType;
            std::vector<Type*> vec;
            vec.emplace_back(TypeSystem::floatType);
            funcType = new FunctionType($1,funchead->getType(), vec);
            se = new IdentifierSymbolEntry(funcType, funchead->getName(), 0);
            globals->install(funchead->getName(), se);
            unit.insertDeclare(se);
    ...
    | PUTARRAY LPAREN FuncRParam RPAREN{
        ...
        if(se==nullptr){
            ...
            std::vector<Type*> vec;
            vec.emplace_back(TypeSystem::intType);
            //fix:vec.emplace_back();这里需要补一个int数组类型的函数参数
            //z1209 完善指针
            PointerType *arrType =new PointerType(TypeSystem::intType);
            vec.emplace_back(arrType);
            funcType = new FunctionType($1,funchead->getType(), vec);
    ...
    | PUTFARRAY LPAREN FuncRParam RPAREN{
        ...
        if(se==nullptr){
            ...
            std::vector<Type*> vec;
            vec.emplace_back(TypeSystem::intType);
            //fix:vec.emplace_back();这里需要补一个float数组类型的函数参数
            //z1209 完善指针
            vec.emplace_back(new PointerType(TypeSystem::floatType));
            funcType = new FunctionType($1,funchead->getType(), vec);
            se = new IdentifierSymbolEntry(funcType, funchead->getName(), 0);
            globals->install(funchead->getName(), se);
            unit.insertDeclare(se);

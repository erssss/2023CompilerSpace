...
void BinaryExpr::genCode()
{
    ...
    if (op == AND)
    {
        ...
        if (endBB->empty())
        {
            ...
            BinaryInstruction*binary = new BinaryInstruction(op, temp, tem1, tem2, builder->getInsertBB());
    ...
    else if (op >= LESS && op <= NOTEQUAL)
    {
        ...
        if (type1->isFloat() && type2->isInt()) {
            ImplictCastExpr* temp =
                ...
                // temp->push_back_opse(expr2->getSymbolEntry());
            this->expr2 = temp;
        } else if (type1->isInt() && type2->isFloat()){
            ImplictCastExpr* temp =
                new ImplictCastExpr(expr1, TypeSystem::floatType);
                // temp->push_back_opse(expr1->getSymbolEntry());
            this->expr1 = temp;
        ...
        if (src1->getType() == TypeSystem::boolType)
        {
            ...
            // ZextInstruction* zext= 
                new ZextInstruction(dst, src1, bb);
            // zext->push_back_opse(expr1->getSymbolEntry());
            src1 = dst;
        }
        if (src2->getType() == TypeSystem::boolType)
        {
            ...
            // zext->push_back_opse(expr2->getSymbolEntry());
            src2 = dst;
        }
        int cmpopcode;
        switch (op)
    ...
    {
        printg("++++++++++++++++++++++++++++++++++++++ADD");
        SymbolEntry* se_tmp1=expr1->getSymbolEntry();
        SymbolEntry* se_tmp2=expr2->getSymbolEntry();
        Type *type1 = getBasicType(expr1->getSymbolEntry());
        Type *type2 = getBasicType(expr2->getSymbolEntry());
        if (type1->isFloat() && type2->isInt()) {
            ImplictCastExpr* temp =
        ...
        expr1->genCode();
        Operand *src1 = expr1->getOperand();
        // if(expr1->getSymbolEntry()->getType()->isArray()){
        //     auto idx_se=src1->getIdxSe();
        //     for(auto idx:idx_se){
        //     std::cout<<"+++++++++++++++"<<idx->toStr()<<"\n";
        //     }
        // }
        expr2->genCode();
        Operand *src2 = expr2->getOperand();
        // if(expr2->getSymbolEntry()->getType()->isArray()){
        //     auto idx_se=src2->getIdxSe();
        //     for(auto idx:idx_se){
        //     std::cout<<"+++++++++++++++"<<idx->toStr()<<"\n";
        // }
        // }
        ...
        BinaryInstruction * binary = new BinaryInstruction(opcode, dst, src1, src2, bb);
        binary->push_back_opse(se_tmp1);
        binary->push_back_opse(se_tmp2);
    }
    printg("------------------------------------BinaryInstruction");
}
...
void Constant::genCode()
{
    ...
    if(symbolEntry==nullptr){
        std::cout<<"??\n";
        symbolEntry=dst->getSymbolEntry();
    }
    ...
    else
    {
        ...
        if (arrFlag || idx == nullptr)
        {
            ...
            else
            {
                ...
                // gep->push_back_opse(expr1->getSymbolEntry());
                gep->setIdxFirst(1);
            }
        ...
        for (; idx != nullptr;)
        {
            ...
            // std::cout<<"---------------------"<<idx->getSymbolEntry()->toStr()<<"\n";
            // if(idx_se==nullptr){
                // idx_se=idx->getSymbolEntry();
            // }
            // else
                // idx_se->append(idx->getSymbolEntry())
            // }
            ...
            GepInstruction *gep;
            if(se->isParam()){
                gep = new GepInstruction(indice_dst, indice_src, bb, idx->getOperand(),1,addr->getSymbolEntry());
            }
            else
            {
                gep = new GepInstruction(indice_dst, indice_src, bb, idx->getOperand());
...
void FuncCall::genCode()
{
         ...
         paramExpr = dynamic_cast<ExprNode *>(paramExpr->getNext()),cnt++)
    {
        ...
        for (auto it = range.first; it != range.second; ++it) {
            if(it->second->paramno==cnt&&it->second->isTrans){
        ...
        paramExpr->genCode();
        // param_on = 0;
        rParams.emplace_back(paramExpr->getOperand());
        auto tmp=(*(recent_op.end()-2));
        if(notEleDimen){
            // std::cout<<"---------- "<<tmp->toStr()<<"\n";
            tmp_op.emplace_back(tmp);
        }
        notEleDimen=0;
    }
    ...
    // }else{
        ...
        dst = new Operand(ret);
    // }
    FuncCallInstruction* call = new FuncCallInstruction(dst, symbolEntry, rParams, entry);
    // auto tmp=(*(recent_op.end()-3));
    // std::cout<<"??? "<<tmp->toStr()<<"\n";
    // tmp->addUse(call);
    for(size_t i=0;i<tmp_op.size();i++){
        tmp_op[i]->addUse(call);
    }
    call->getOpse().emplace_back(ret);
}
...
void UnaryExpr::genCode()
{
    ...
    if (op == NOT)
    {
        ...
        xorInstr->push_back_opse(expr1->getSymbolEntry());
    }
    else
    {
...
void InitStmt::genCode()
{
    ...
    if (is_init && expr != nullptr)
    {
            ...
            while (expr != nullptr)
            {
                ...
                if (expr->getNull())
                {
                    if (first_in && expr->getNext() == nullptr)
                    {
                        ...
                        Operand *tmp = new Operand(new TemporarySymbolEntry(int8PtrType, SymbolTable::getLabel()));

                        // Operand*dst = new Operand(new TemporarySymbolEntry(eletype, SymbolTable::getLabel()));
                        // new LoadInstruction(dst, addr, bb);
                        BitcastInstruction *bitcast=new BitcastInstruction(tmp, se->getAddr(), bb);
                        bitcast->push_back_opse(se);
                        if (func_se == nullptr){
                            std::vector<Type*> vec;
                            vec.emplace_back(int8PtrType);
                            vec.emplace_back(TypeSystem::int8Type);
                            vec.emplace_back(TypeSystem::intType);
                            vec.emplace_back(TypeSystem::boolType);
                        ...
                        params.emplace_back(new Operand(
                            new ConstantSymbolEntry(TypeSystem::int8Type, 0)));
                        params.emplace_back(new Operand(
                            new ConstantSymbolEntry(TypeSystem::intType, cnt*(arrType->getEleType()->getSize())/8)));
                        params.emplace_back(new Operand(
                ...
                else
                {
                    ...
                    for (int i = 0; i < dimen; i++)
                    {
                        ...
                        // idx_stack.emplace_back(tmp_idx);
                    }
                    ...
                    // store->push_back_opse(se->getSymbolEntry());
                ...
                while (allInitZero || (expr->getLast()))
                {
                    ...
                    arrType->getValueVec().emplace_back(0);
        ...
        else
        {
            ...
            // store->push_back_opse(id->getSymbolEntry());
        }
    }
}
void AssignStmt::genCode()
{
    ...
    // std::cout<<"addr   =   "<<addr->toStr()<<"\n";
    // addr->addUse(store);
    // if(expr->getSymbolEntry()->getType()->isArray())
    // if(array_button){
    //     arr_op->addUse(store);
    //     std::cout<<"arr_op   =   "<<arr_op->toStr()<<"\n";

    // }
    // else
    // 这里需要改
    (*(recent_op.end()-2))->addUse(store);
    store->push_back_opse(lval->getSymbolEntry());
...
void ImplictCastExpr::genCode()
{
    ...
    if (type == TypeSystem::boolType) {
        ...
        this->trueList().emplace_back(
            new CondBrInstruction(trueBB, tempbb, this->dst, bb));
        this->falseList().emplace_back(new UncondBrInstruction(falseBB, tempbb));
    }else if (type->isInt()) {
        FptosiInstruction* ftoi = new FptosiInstruction(dst, this->expr->getOperand(), bb);
        ftoi->push_back_opse(expr->getSymbolEntry());
    } else if (type->isFloat()) {
        SitofpInstruction*stof = new SitofpInstruction(dst, this->expr->getOperand(), bb);

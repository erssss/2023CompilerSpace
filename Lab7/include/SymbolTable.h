...
// z0110->0114 函数实参隐式类型转换链接->修正paramno
struct transItem{
    bool isTrans=0;
...
public:
    ...
    void appendTransItem(std::string func_name,transItem* item){
        transitems.insert(pair<string, transItem*>(func_name,item));
    };
...
public:
    ...
    std::string toStr();
    void setIdxSe(SymbolEntry*idx_ses){this->idx_ses=idx_ses;}
    SymbolEntry*& getIdxSe(){return idx_ses;}

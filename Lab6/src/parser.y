...
LVal
    : ID {
        ...
        if(se == nullptr)
        {
            ...
            if(se->getType()->isArray()){
                ...
                // arrType->setFlag(1);
                // se->setType(new PointerType(arrType->getEleType()));
                // se=new IdentifierSymbolEntry(new PointerType(arrType,0),$1,dynamic_cast<IdentifierSymbolEntry*>(se)->getScope());
    ...
    | ID ArrayIndices{
        ...
        if(sliceCnt!=arrType->getDimen()){
            //z1209 fix 这里的scope有待商榷
            se=new IdentifierSymbolEntry(new PointerType(arrType,sliceCnt),$1,dynamic_cast<IdentifierSymbolEntry*>(se)->getScope());
            // if(check_on){
            //     fprintf(stderr, "High-dimen array is not assignable!\n");
            //     if(assert_on)assert(sliceCnt==arrType->getDimen());
            // }
        }
        // z1209 完善数组
        $$ = new Id(se,$2,$1,1);
...
/* z1214 dbq 还是得改 */
    : ID ASSIGN Cond{
        ...
        // std::cout<<"已经初始化1"<<se->toStr()<<std::endl;
        if(const_flag){
    ...
    InitStmt SEMICOLON{
        ...
        while(initexp!=nullptr){
            ...
            if(initexp->getId()->getIsArray()){
                ...
                // std::cout<<cs->getType()->toStr()<<cs->getFloatValue()<<std::endl;
                // if($3 == TypeSystem::intType){
                //     if(cs->getType() == TypeSystem::intType){
                //         idse->setIntValue(cs->getIntValue());
                //     }else{
                //         idse->setIntValue(cs->getFloatValue());
                //         std::cout<<"已经计算const "<<idse->toStr()<<" "<<idse->getIntValue()<<std::endl;
                //     }
                //     // std::cout<<dynamic_cast<ConstantSymbolEntry*>(expr->getSymbolEntry())->getIntValue()<<std::endl;
                // }else{
                //     if(cs->getType() == TypeSystem::intType){
                ...
                //     }
                //     // idse->setFloatValue(dynamic_cast<ConstantSymbolEntry*>(expr->getSymbolEntry())->getFloatValue());
                // }
                // idse->setIdtype(IdentSystem::constant); 
            }
...
BreakStmt
    : BREAK SEMICOLON {
        ...
        if(now_whilestmt==nullptr)
        std::cout<<"?????????????????????????????????\n";
        $$ = new BreakStmt(now_whilestmt);
    }
    ;
ContinueStmt
    : CONTINUE SEMICOLON {
...
ReturnStmt
    ...
    RETURN Cond SEMICOLON{
        ...
        std::vector<Type*> typeVec;
        Type* type=$2->getSymbolEntry()->getType();
        typeVec.push_back(retType);
        typeVec.push_back(type);
        int retno = pairTypeCheck(typeVec);
    ...
    RelExp LESS AddExp
    {
        ...
        if($1->getSymbolEntry()->isConstant()&&$3->getSymbolEntry()->isConstant()){
            ...
            if(ts1->getType() == TypeSystem::floatType && ts2->getType() == TypeSystem::floatType){
                ...
                cs = new ConstantSymbolEntry(TypeSystem::boolType,ts1->getIntValue()<ts1->getFloatValue());
            }
            else{
...
FuncCall
    ...
    | GETARRAY LPAREN FuncRParam RPAREN{
        ...
        if(se==nullptr){
            ...
            //z1209 完善指针
            vec.push_back(new PointerType(TypeSystem::intType));
            funcType = new FunctionType(funchead->getType(), vec);
            se = new IdentifierSymbolEntry(funcType, funchead->getName(), 0);
            globals->install(funchead->getName(), se);
            unit.insertDeclare(se);
